    ...
    def validate_aadhar_number(self, value):
        ...
        if Savings.objects.filter(aadhar_number=value).exists():
            raise ValidationError(
                "This Aadhar number is already associated with another account."
            )
        return value
...
class DepositSerializer(serializers.Serializer):
    ...
    )  # Description field added to the serializer
...
class FundTransferSerializer(serializers.ModelSerializer):
    ...
    def get_account_type(self, obj):
        ...
        try:
            sender_account = Savings.objects.get(
                account_number=obj.sender_account_number
            )
...
class ExpenseSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            raise serializers.ValidationError(
                f"No budget found for category '{category_name}'."
            )
